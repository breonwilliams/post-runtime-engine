...
- **Recursive option writes: every grouping save spawned ~20 junk autoloaded options.** `PCPTPages_Renderer::maybe_bump_groupings_changed()` is hooked to the GENERIC `updated_option` / `added_option` / `deleted_option` actions and fires for any option under `PCPTPages_Grouping_Registry::OPTION_PREFIX` (`pcptpages_groupings_`). The cache-invalidation marker it wrote — `pcptpages_groupings_changed_{slug}` — sat under that same prefix, so writing it RE-ENTERED the hook, which derived a slug of `changed_{slug}` and wrote a marker one level deeper, recursing until the `option_name` column's 191-char limit stopped it.
  ...
  Cleaned up automatically. The rename stops new rows but cannot reach the ones already on disk, and a site that ran the old code keeps reading them — autoloaded — on every request forever. Data version 0.7.0 therefore carries the first cleanup migration in this plugin's history that REMOVES rows rather than marking a shape change; it runs once, on the first admin or CLI request after upgrade.

  Deletion is matched on the option's VALUE, not its name, because the name cannot tell the two apart: a CPT whose slug legitimately begins with `changed` — `changed_events` — stores its groupings at `pcptpages_groupings_changed_events`, which is exactly the pattern the junk took. Real grouping definitions are an array; the junk markers were a bare `time()` integer. Anything else is left alone, deliberately: keeping a junk row costs a few bytes per request, while deleting a real one destroys grouping definitions that exist in no other copy. The predicate is `PCPTPages_Renderer::is_legacy_recursive_marker_value()`, covered by 13 unit tests including the collision case and the `(string) true === '1'` trap.

  Rows go through `delete_option()` rather than a bulk `DELETE`. A direct query would leave the alloptions cache holding every row just removed — the same stale-cache behaviour that forced the purge fix below to move its `delete_option()` call after `unregister()`.

  Verified on a live install: three planted junk rows (including two- and three-level recursions) deleted, a planted `changed_events` groupings array left intact, the deleted names absent from `wp_load_alloptions()`, and a second run with the version rewound changing nothing. The first read after upgrade sees no marker and re-renders each cached post once — harmless.
...
- **Location / map post-field display type.** A new `location` display type turns a per-post address string into a click-to-load map on the single-post page — **no Google Maps API key, no latitude/longitude, no geocoding.** **PRE renders the map itself** — its own `assets/css/map.css` + `assets/js/pre-map.js` — so it works **with or without Promptless WP**; when Promptless WP is active the map picks up the brand palette automatically (parity via the shared `--aisb-*` tokens, consumed with literal fallbacks, exactly like the gallery lightbox — no runtime dependency and no shared code). Cards and archives show the address as plain text. Per-field options: `map_zoom` (street/neighborhood/city), `map_load` (auto/click — click is the privacy-friendly default), and `show_directions`. When a post has no address, the map falls back to the site's Business Identity address, read directly from the shared `aisb_business_settings` option. Full design: `docs/LOCATION_MAP_DESIGN.md`.
  - **Repositionable like a grouping.** A map is block-level, so it is placed with the SAME `above_main` / `below_main` / `sidebar` vocabulary groupings use — not the inline hero slots scalar fields use. The field carries a default placement (`map_position`, default `below_main`; `hidden` suppresses the map), and — exactly like a grouping's per-post `position` override — each post can move its map (or hide it) from the post editor via a "Map placement" dropdown, stored in the per-post field-override store. The renderer buckets maps into the main column (above/below the content) or the sidebar, switching the page to the with-sidebar layout when anything is placed there. Verified live: a session map rendered correctly in the main column and, via the per-post override, in the sidebar.
  - **Manual authoring UX.** The post-field editor offers "Location / map (address)" with zoom / load / directions controls, and the post editor shows an address box with a hint — so a non-technical client can create and fill location fields entirely by hand, not only through the connector.
  - **Connector parity.** `location` and its `map_zoom` / `map_load` / `show_directions` / `map_position` attributes (plus the `map_zoom_levels`, `map_load_modes`, and `map_positions` enums and a `location_map_setup` critical rule) are surfaced across the REST preflight, the MCP relay schema, and the tool allow-lists, so AI-driven authoring can define and populate location fields. The per-post map placement override rides the existing `set_post_field_visibility` endpoint (`{ field_key: { map_position } }`).
...
- Overlay-hero badge collision fix; CSS versioned by file mtime
- In-flow `image_overlay` treatment on AISB PostGrid compact rows
