...
**Document status:** Draft — scope-expansion contract for a location/map display on registered CPTs. Locks the architectural decisions before any code is written. Implementations MUST follow this design. Disagreements are resolved by editing this doc, not by writing different code.
