	...
	private static function get_critical_rules() {
		return array(
			...
			'featured_card_max_one'        => 'featured-card variant has max_items=1 enforced by the validator. featured-card is for ONE prominent item per grouping (a Lead Architect, a Schedule a Tour CTA, a Currently Featured project). For multi-item collections of cards-with-images use card-grid.',
				'gallery_variant'              => 'The `gallery` variant renders a grouping\'s items as a responsive photo grid (3-up desktop, 2-up mobile; 16:9 tiles by default — see gallery_image_aspect below) with an accessible lightbox (keyboard navigation, prev/next buttons, swipe, focus trap). USE IT for supporting-photo collections on listing-style posts: property photos, vehicle galleries, portfolio shots, venue interiors. ITEM SHAPE: image_id (required to render — items without one are valid but SKIPPED) + optional heading (rendered as the photo caption under the tile and in the lightbox). icon_id is REJECTED by the validator on gallery items; supporting_text / link / link_post_id are accepted but NOT rendered (the tile\'s single interaction is opening the lightbox). SET ALT TEXT on the attachments when uploading (update the attachment alt field) — the gallery reads alt from the attachment, and images without alt render as decorative. ~24 items or fewer reads best (no hard cap; performance is protected by lazy-loading + on-demand full-size loading). AUTO-SOURCES work: a child_posts / taxonomy_match / meta_match gallery pulls each matched post\'s FEATURED IMAGE as the tile (posts without one are skipped) — e.g. a Building page with a gallery of its Unit photos via child_posts. Groupings with default_variant "gallery" should usually also set heading_required:false in the definition so image-only tiles save cleanly from the admin meta box too. TILE CROP: the definition-level gallery_image_aspect field ("16:9" default | "4:3" | "1:1" | "4:5" — same vocabulary as archive_image_aspect) sets the tile crop shape; 4:3 suits property/vehicle photography, 1:1 suits product shots, 4:5 suits portraits. The lightbox always shows the full uncropped image regardless.',
...
WHOSE CONTENT. delete_post refuses (403 pcptpages_foreign_post_type) any post whose type belongs to WordPress core or another plugin — post, page, product, attachment, another plugin\'s CPTs. Orphan status is proven by a TOMBSTONE this plugin writes when it unregisters a CPT, NOT by the naive test of "not registered anywhere", which would also match another plugin\'s temporarily-unregistered types and let allow_unregistered destroy their data. Consequence worth knowing: posts orphaned BEFORE the tombstone existed are not listed, because nothing records that they were ever ours — those need manual cleanup.',
	...
	private static function get_field_name_hints() {
		return array(
			...
			'grouping_definition'  => array( 'key', 'label', 'description', 'default_variant', 'default_position', 'default_source', 'max_items', 'heading_required', 'supporting_text_required', 'link_required', 'icon_or_image_required', 'gallery_image_aspect' ),
			'post_field_definition' => array( 'key', 'label', 'description', 'display_type', 'card_position', 'single_position', 'color_intent', 'icon', 'options', 'required', 'date_format', 'date_format_string', 'currency_code', 'value_suffix', 'max', 'unit_label', 'number_grouping', 'semantic_role', 'all_day', 'event_timezone', 'filterable', 'sortable', 'filter_widget', 'map_zoom', 'map_load', 'show_directions', 'map_position' ),
			'notes'                => 'icon_id and image_id are mutually exclusive on a single item. featured-card has max_items=1 enforced. Compact-grid and horizontal-row are icon-only — image_id is dropped at render time. GALLERY: gallery-variant items take image_id + optional heading (rendered as the photo caption) ONLY — icon_id is REJECTED by the validator, and supporting_text/link/link_post_id are not rendered (the tile click opens the lightbox); an item without image_id is valid but skipped at render. See critical_rules.gallery_variant. link_post_id is preferred over literal `link` URLs for internal references; both can be set (link is the fallback when link_post_id resolution fails). Post field definition fields not in post_field_definition above are silently dropped on write by PCPTPages_Validator; conditional fields (color_intent, options for badge; icon for meta_pair; date_format / date_format_string for date; currency_code for currency; max + unit_label for rating / progress / number_with_label; number_grouping for number_with_label; map_zoom / map_load / show_directions for location) are only meaningful when paired with the right display_type. LOCATION: the location display type renders a per-post address as a click-to-load map on the single-post page (no API key / coordinates); see critical_rules.location_map_setup and post_field_enums.map_zoom_levels / map_load_modes. EVENTS: semantic_role (see post_field_enums.semantic_roles) tags a field for the event archive filter + Event schema; all_day and event_timezone apply to date fields (see post_field_enums.date_field_event_attributes). See critical_rules.events_archive_setup for the full workflow. FILTERS: set filterable:true to let visitors filter the archive by this field (the widget is auto-chosen from display_type) and sortable:true to offer it as a sort option; filter_widget optionally overrides the widget (see post_field_enums.filter_widgets). meta_pair cannot be filterable/sortable. See critical_rules.filterable_archive_setup for the full two-plugin workflow (declare filterable fields here, then enable_filters on a Promptless PostGrid).',
		);
	}
	...
	private static function get_post_field_enums() {
		return array(
			// 10 display types with example renderings — the labels mirror
			...
			'display_types' => array(
				...
				array( 'value' => 'location',          'label' => 'Location / map (address)',        'description' => 'An address string. The single-post page renders a click-to-load Google map built from the address — NO API key, NO coordinates, NO geocoding. Promptless CPT Pages renders the map itself (its own map.css + pre-map.js), so it works with OR without Promptless WP — no dependency. Cards / archives show the address as plain text. Per-field map options: map_zoom (street|neighborhood|city), map_load (auto|click, default click = privacy-friendly), show_directions (bool), and map_position — the block-level placement of the map on the single-post page (above_main|below_main|sidebar|hidden), the SAME vocabulary groupings use, default below_main. Each post can OVERRIDE the placement (and hide the map) via set_post_field_visibility ({ field_key: { map_position } }), exactly as a grouping\'s per-post position overrides its default. Empty address falls back to the shared Business Identity address (read from the aisb_business_settings option) at render; the map inherits the brand palette when Promptless WP is active. Set the address per post via set_post_field_values. Display-only (not filterable/sortable). See critical_rules.location_map_setup and post_field_enums.map_zoom_levels / map_load_modes / map_positions.' ),
			),
			...
			'filter_widgets' => array(
				...
				'location'          => 'not filterable (display-only)',
			),
			// Location / map (v1.3): named zoom levels for the location display
			// type. The named key crosses to Promptless WP's Map section, which
			// owns the numeric Google Maps zoom — so the two never drift.
			'map_zoom_levels' => array(
				array( 'value' => 'street',       'label' => 'Street (closest)', 'description' => 'Tightest framing — a single building / storefront.' ),
				array( 'value' => 'neighborhood', 'label' => 'Neighborhood',     'description' => 'Default. Shows the address in its surrounding blocks — good for most listings and offices.' ),
				array( 'value' => 'city',         'label' => 'City (widest)',    'description' => 'Wide framing — the address within its city / metro.' ),
			),
			// Location / map (v1.3): load behavior for the location display type.
			'map_load_modes' => array(
				array( 'value' => 'click', 'label' => 'Click to load (default)', 'description' => 'Privacy-friendly. Shows a token-styled facade; contacts Google only when the visitor clicks. Lighter on card-heavy pages.' ),
				array( 'value' => 'auto',  'label' => 'Load automatically',      'description' => 'Lazily loads the map iframe on paint. Use when an always-visible map is wanted.' ),
			),
			// Location / map (v1.3): block-level placement of the map on the
			// single-post page — the SAME above_main/below_main/sidebar
			// vocabulary groupings use (plus `hidden`). Set on the field as the
			// default (`map_position`); overridable per post via
			// set_post_field_visibility ({ field_key: { map_position } }).
			'map_positions' => array(
				array( 'value' => 'above_main', 'label' => 'Above content', 'description' => 'Map renders in the main column, above the post content.' ),
				array( 'value' => 'below_main', 'label' => 'Below content', 'description' => 'Default. Map renders in the main column, below the post content.' ),
				array( 'value' => 'sidebar',    'label' => 'Sidebar',       'description' => 'Map renders in the single-post sidebar (the page switches to the with-sidebar layout when anything is placed there).' ),
				array( 'value' => 'hidden',      'label' => 'Hidden',         'description' => 'No map on the single-post page (the address still shows as text on cards per card_position).' ),
			),
		...
		return rest_ensure_response( array(
			'variants' => array(
				...
				array(
					...
					'max_items_required'       => null,
					'supports_supporting_text' => false,
					'notes'                    => 'Responsive photo grid with an accessible lightbox. Items take image_id + optional heading (caption) only — icon_id is rejected; supporting_text/link are not rendered. ~24 items or fewer reads best. See critical_rules.gallery_variant.',
				),
			),
		) );
	}
