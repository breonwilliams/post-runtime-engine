...
> - **Location & map display** — `docs/LOCATION_MAP_DESIGN.md`. **✅ IMPLEMENTED 2026-08-20.** A `location` post-field display type that ports Promptless WP's address-based Map section (no API key, no coordinates) to per-post maps. Shipped end-to-end: validator amendment + guard tests, the shared `aisb_render_map_embed` filter / `aisb_enqueue_map_embed_assets` action on the AISB side, the single-post map block + card address-as-text + business-identity fallback + graceful degradation, full admin UX for manual authoring, and connector/relay/preflight parity. See the implementation note at the top of the design contract for the one clarification (map renders as a block below content; `single_position` acts as show/hide).
