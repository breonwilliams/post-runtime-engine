...
**v1.1 post fields (SHIPPED):** second field type — scalar **post fields** with a closed enum of display types — 9 at v1.1 (currency, number_with_label, badge, meta_pair, date, text, rating, progress, multi_badge), 10 since `location` was added 2026-08-20; `PCPTPages_Validator::DISPLAY_TYPES` is the list and 5 positions symmetric across single-post hero and card contexts (image_overlay, headline, subtitle, meta_strip, footer_meta, plus `hidden`). Implemented in `PCPTPages_Post_Field_Registry`, `PCPTPages_Card_Renderer`, `PCPTPages_Meta_Box_Post_Fields`, with full REST/MCP CRUD + reorder + per-post values/visibility. Design contract: **[`docs/POST_FIELDS_V1_1_DESIGN.md`](docs/POST_FIELDS_V1_1_DESIGN.md)**. (Those 5 `FIELD_POSITIONS` are the *inline* placement slots; a display type can instead be *block-level* and use the grouping placement model — see "Two placement classes" under the maintenance constraints. `location` is the first block-level display type.)

**v1.2 layer (SHIPPED, post-dates the phase table below):** events vertical (`PCPTPages_Event_Query`, `PCPTPages_Event_Schema` Schema.org JSON-LD, `PCPTPages_Event_Calendar` iCalendar feed + "Add to calendar" on core `add_feed('ics')` — 2026-09-12, `EVENTS_VERTICAL_DESIGN.md` §14, semantic date roles with `__sort`/`__utc` companion meta), schema-driven filters (`PCPTPages_Filter_Descriptors`, `filterable`/`sortable`/`filter_widget` field attributes, `aisb_postfilter_query_args` integration), AISB PostGrid / theme-archive card hooks (`PCPTPages_Card_Filter_Hooks` listening to `promptless_archive_card_section` + `aisb_postgrid_card_section`), and SEO meta tags (`PCPTPages_Meta_Tags`). Design docs: `docs/EVENTS_VERTICAL_DESIGN.md`, `docs/POSTGRID_PREVIEW_PARITY.md`.

**Shipped since 0.6.6 (this header is not re-versioned every change — trust `CHANGELOG.md` for exact versions/rationale):** the `gallery` grouping variant (photo grid + accessible lightbox; `docs/GALLERY_VARIANT_DESIGN.md`); a **`location` display type** — the first *block-level* post-field display type — that renders a keyless, click-to-load map (built from the address string — no API key or coordinates) that **PRE renders itself** — its own `assets/css/map.css` + `assets/js/pre-map.js`, styled with `--aisb-*` tokens plus literal fallbacks — so it works **with OR without Promptless WP** (no runtime dependency; brand parity comes from shared tokens, exactly like the gallery lightbox). Repositionable with the grouping placement model (`map_position` = `above_main`/`below_main`/`sidebar`/`hidden`, per-post override); an empty address falls back to the business-identity address read directly from the shared `aisb_business_settings` option (`docs/LOCATION_MAP_DESIGN.md`); the per-post field-visibility override store (`_pcptpages_field_visibility`) generalized to also carry block-level placement; a single-post hero-width fix; and three forward-looking design contracts placed for future build (`docs/RELATIONSHIP_FIELDS_V1_2_DESIGN.md`, `docs/CSV_IMPORT_DESIGN.md`, `docs/LOCATION_MAP_DESIGN.md`). **See the "Two placement classes" constraint below before adding any new display element** — it is the pattern the location work established (and the first map build missed).
...
2. **One repeatable field type ("grouping"), one item shape.** No dates, numbers, taxonomy fields, nested groups, conditional logic, or relationship fields in v1.0. Just `{image-or-icon, heading, supporting_text, optional link}`. The constraint is the feature. Each grouping supports per-post variant override (4 variants: compact-grid, card-grid, featured-card, horizontal-row) and four source modes (manual, child_posts, taxonomy_match, meta_match) for auto-population. (v1.1 later added the second, deliberately-scoped scalar "post fields" type — see status header.)
...
See `docs/ROADMAP.md` for full phase detail and per-phase changelog. Current shipped version: **v0.10.0** (`PCPTPages_VERSION`), data-schema version 0.7.0 (`PCPTPages_DATA_VERSION`) — read the constants in `post-runtime-engine.php` rather than trusting this line. **Outstanding work before v1.0 ship** is documented in `POST_RUNTIME_AUDIT.md` (test coverage scaffold, doc-spec drift on connector preflight fields, optional production-polish items).
...
**One-line summary:** update version stamps in `post-runtime-engine.php` (header + `PCPTPages_VERSION` constant), `readme.txt` (Stable tag + Changelog + Upgrade Notice), and `CHANGELOG.md` → commit "Release X.Y.Z" → push → `./bin/build-release.sh` → `wp plugin check build/promptless-cpt-pages` → `git tag vX.Y.Z && git push --tags` → `gh release create vX.Y.Z build/promptless-cpt-pages.zip --title "vX.Y.Z" --generate-notes` → publish `build/promptless-cpt-pages/` to WordPress.org SVN trunk + `tags/X.Y.Z` (see RELEASE.md for exact rsync/svn commands). **Attach the built `promptless-cpt-pages.zip` to the GitHub release.** WordPress.org is the update channel (the GitHub auto-updater was retired per WP.org guideline #8, which is about updaters, not about release assets), but the release page is where people download from, and without the built ZIP its only download is GitHub's source archive — a folder named `post-runtime-engine-<tag>` that installs as a SECOND plugin and fails Plugin Check with ~850 false text-domain errors. That happened twice on 2026-09-12 because this line used to say the opposite of `RELEASE.md`. The build script verifies the ZIP's internal structure and aborts on a flattened/hand-assembled archive — never package a release by hand.
...
- **Two placement classes for display elements — decide the class FIRST, before writing any render code.** Everything PRE draws on a single-post page is either *inline* or *block-level*, and that choice dictates which placement enum and which machinery it uses. This is the single most missable design decision in the plugin, so it is spelled out here rather than left implicit in the two enums:
  - **Inline** = metadata that sits *inside the hero or the card* (a badge, a date, a rating, a currency value). It uses `PCPTPages_Validator::FIELD_POSITIONS` — the five hero/card slots `image_overlay`, `headline`, `subtitle`, `meta_strip`, `footer_meta` (+ `hidden`), symmetric across single-post hero and card contexts. Scalar post fields are inline.
  - **Block-level** = a *standalone block in the body flow* (a grouping; a location map). It uses the grouping placement model: `PCPTPages_Validator::POSITIONS` (`above_main`, `below_main`, `sidebar`) **with a per-post override**, and placing one in the sidebar switches the page to the two-column layout. A block-level element MUST reuse that machinery — bucket-by-position in the renderer, per-post placement in the field-visibility override store (`_pcptpages_field_visibility`) exactly like a grouping's per-post `position`. **Never hardcode a block-level element into one fixed spot in the template.**
  - **`location` is the proof this distinction matters:** it is a *post-field display type* (so instinct says "inline, use FIELD_POSITIONS") but it is *block-level by layout*. The first map build missed this — it shipped below-content-only with no sidebar path — and the fix was to give it the grouping placement model. "Post field" does NOT imply "inline." Ask the question every time.
  - **Adding a new display element — answer these in its `docs/*_DESIGN.md` contract before coding:** (1) inline or block-level? (2) if block-level, it takes `above_main`/`below_main`/`sidebar` + a per-post override + sidebar-triggers-two-column, identical to a grouping — reuse, don't reinvent; (3) which closed enum(s) grow, and extend the guard test **and** the connector vocabulary (REST allowlists, relay schema + body-field lists, preflight, knowledge map) in the SAME change set; (4) how it degrades when Promptless WP is inactive.
  - Both position enums are closed and owned by `PCPTPages_Validator`. Resist adding a *new position value* to either without an architectural conversation — but note that adding a *block-level element* is not adding a position; it reuses the existing three. (`PCPTPages_Validator` is the single source of truth for every closed enum: `VARIANTS`, `POSITIONS`, `MAP_POSITIONS`, `SOURCE_MODES` — four incl. `meta_match` — `DISPLAY_TYPES`, `FIELD_POSITIONS`, `COLOR_INTENTS`, `SEMANTIC_ROLES`, `FILTER_WIDGETS`, plus the map enums `MAP_ZOOM_LEVELS` / `MAP_LOAD_MODES`.)
